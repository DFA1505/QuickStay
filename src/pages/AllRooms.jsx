...
const AllRooms = () => {
  ...
  const toggleType = (name) => {
    if (selectedTypes.includes(name)) {
      setSelectedTypes((prev) => prev.filter((item) => item !== name));
    } else {
  ...
  return (
    <div className="flex flex-col-reverse lg:flex-row items-start justify-between pt-28 md:pt-35 px-4 md:px-16 lg:px-24 xl:px-32">
      ...
      <div className="bg-white lg:w-80 border border-gray-300 text-gray-600 max-lg:mb-8 lg:mt-16 w-full">
        <div
          ...
          <div className="px-5 pt-5 pb-7">
            ...
            {priceRanges.map((range, index) => (
              <Checkbox
                key={index}
                label={range}
                selected={selectedTypes.includes(range)}
                onChange={toggleType}
              />
            ))}
          </div>
